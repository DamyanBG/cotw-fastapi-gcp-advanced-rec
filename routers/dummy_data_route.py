...
from dummy_data.data_creators import create_dummy_user, create_dummy_nrc, create_dummy_crc, create_dummy_cotw
...
async def create_dummy_data(number_to_create):
    ...
    for _ in range(1, number_to_create):
        ...
        async with asopen(f"dummy_data/images/cat_image_{image_number}.webp", "rb") as f:
            image_data = await f.read()
        image_base64 = base64.b64encode(image_data).decode("utf-8")
        base64_with_data_url = f"data:image/webp;base64,{image_base64}"

        image_bytes = compress_image_to_webp(separate_data_url_from_base64(base64_with_data_url)[1])
    ...
    insert_image_tasks = [insert_image(image_name) for image_name in cat_images_file_names]
    images = await asyncio.gather(*insert_image_tasks)

    # Create dummy current round cats
    crc_creates = [create_dummy_crc(user.id, image.id) for user, image in zip(users, images)]
